...
import com.muzisoft.division.utils.converter.PointSupplierConverter;
...
public class UserPoints extends BaseTimeEntity {
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "seq_manager_user_points")
    @GenericGenerator(name = "seq_manager_user_points", strategy = "com.muzisoft.division.utils.manager.SeqManager", parameters = {
            @org.hibernate.annotations.Parameter(name = SeqManager.VALUE_USER_SEQ_PARAMETER, value = "uspt_"),
    ...
    @Convert(converter = PointSupplierConverter.class)
    @Column(nullable = false, columnDefinition = "tinyint(1)")
    private PointSupplier supplier;
    ...
    public static UserPoints create(UserDetails userDetails, PointSupplier supplier, int savedAmount) {
        UserPoints userPoints = new UserPoints();

        userPoints.setUserDetails(userDetails);
        userPoints.setSupplier(supplier);
        userPoints.setSavedAmount(savedAmount);
        userPoints.setUsable(true);
        return userPoints;
    }
